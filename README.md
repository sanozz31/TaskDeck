...
**v1.2.3 — 通知开启体验打磨 · 首次引导不纠缠 / 设置内一键直达**（承 v1.2.2 接入原生通知通道），端到端闭环完整。以下按能力域概览，详细功能与版本演进见 `CHANGELOG.md` 和 `doc/milestones.md`。
...
- **DDL 系统提醒（双通道）**：截止前 24 小时、前 6 小时各弹一次系统通知——桌面端走 Tauri 原生通知插件（macOS WKWebView 不支持 Web Notification），浏览器端降级到 Web Notification；首次打开会弹引导（点「开启提醒」即触发系统授权，「暂不开启」也可随时在「设置 → 通知提醒」点「开启通知」前往开启）。纯日期任务（无具体时刻）锚定当天 23:59；超过截止 24h 不再补提醒避免刷屏；`localStorage` 去重防重复弹窗。**关闭 App 不提醒**为当前 MVP 限制。

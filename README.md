...
**v1.2.0 — 对话卡操作栏 · 编辑/重试/版本切换**（承 v1.1.x 归档回收站 / 行内编辑 / 滚轮），端到端闭环完整。以下按能力域概览，详细功能与版本演进见 `CHANGELOG.md` 和 `doc/milestones.md`。
...
- **全字段行内编辑**：四视图任务卡悬停标题后的 ✎ 或**双击卡片任意区域**进入编辑态，可改标题、描述、标签、优先级、截止日期与时刻；各字段均支持 Enter 保存、Esc 取消（描述 Shift+Enter 换行）。时间轴与悬浮窗任务行支持双击改标题。仅提交真正变化的字段。

...
- **日历视图**：`react-day-picker`，年/月下拉切换、日期网格整体居中、日圆点按当天最高优先级着色。日期格下方有「回到今天」按钮（一键切回当前年月并选中今天）与**当日小记**（随选中日期切换的小记输入框，存本机 localStorage）。**完成日图片**：某天所有任务完成时数字变为一张覆盖图（按日期确定性选定、同一天恒定同一张、跨刷新不变，hover 淡出露数字），往 `src/assets/calendar-overlays/` 加图即自动纳入图片池。

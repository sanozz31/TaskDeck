...
**v1.1.1 — 日历「回到今天」与「当日小记」**（承 v1.1.0 全字段行内编辑 · 全窗口实时同步），端到端闭环完整。以下按能力域概览，详细功能与版本演进见 `CHANGELOG.md` 和 `doc/milestones.md`。
...
- **全字段行内编辑**：四视图任务卡悬停标题后的 ✎ 或双击进入编辑态，可改标题、描述、标签、优先级、截止日期与时刻；时间轴与悬浮窗任务行支持双击改标题。仅提交真正变化的字段。
...
- **日历视图**：`react-day-picker`，年/月下拉切换、日期网格整体居中、日圆点按当天最高优先级着色。日期格下方有「回到今天」按钮（一键切回当前年月并选中今天）与**当日小记**（随选中日期切换的小记输入框，存本机 localStorage）。**完成日图片**：某天所有任务完成时数字变为随机图片覆盖（hover 淡出露数字），往 `src/assets/calendar-overlays/` 加图即自动纳入随机池。
...
| 日历完成日映射 / 当日小记 | 同上（localStorage `taskdeck.calendar.dones` / `taskdeck.calendar.memos`） |

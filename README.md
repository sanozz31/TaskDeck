...
- **DDL 系统提醒（双通道）**：截止前 24 小时、前 6 小时各弹一次系统通知——桌面端走 Tauri 原生通知插件（macOS WKWebView 不支持 Web Notification），浏览器端降级到 Web Notification；首次打开需在系统弹窗中允许通知。纯日期任务（无具体时刻）锚定当天 23:59；超过截止 24h 不再补提醒避免刷屏；`localStorage` 去重防重复弹窗。**关闭 App 不提醒**为当前 MVP 限制。

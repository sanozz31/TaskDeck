...
**v1.1.3 — 删除统一为归档 · 已完成弹窗合并为回收站 · 输入与选择打磨**（承 v1.1.x 行内编辑 / 滚轮 / 回到今天），端到端闭环完整。以下按能力域概览，详细功能与版本演进见 `CHANGELOG.md` 和 `doc/milestones.md`。
...
- **全部任务**：主列表 + 顶栏右侧「已完成任务」入口。任务卡 × 为**软归档**（移出各视图、数据保留）。「已完成」弹窗合并为一列回收站——已完成在上、已归档在下，每张可**恢复**（→待办）或**永久删除**（硬删不可恢复，二次确认）；完成满 7 天自动清除。

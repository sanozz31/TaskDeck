...
**v1.0.0 — 首个正式版**，端到端闭环完整。以下按能力域概览，详细功能与版本演进见 `CHANGELOG.md` 和 `doc/milestones.md`。
...
- **优先级随 DDL 自动升级**：后端定时器（启动 + 每 5 分钟）按距截止远近自动提升开放任务优先级——剩 ≤2h→急、≤24h→高、≤72h→中。**只升不降、幂等**；前端各视图 60s 轮询刷新。
...
| 数据 | 存储位置 | 说明 |
|---|---|---|
| 任务 / 标签库 / 设置 | `~/Library/Application Support/com.taskdeck.desktop/taskdeck.db` | SQLite |
| 对话记录 | WebView localStorage（`taskdeck.chat.v1`） | 聊天历史 |
| 悬浮窗位置/开关 | WebView localStorage（`taskdeck.widget.*`） | 桌面悬浮球状态 |
| 日历完成日映射 | WebView localStorage（`taskdeck.calendar.dones`） | 哪天用什么覆盖图 |
| 输入草稿 / 提醒去重 | WebView localStorage（`taskdeck.chat.draft` / `notified.v2`） | 未发送文字、已弹通知 |

- 卸载或更新 App **不会删除**上述目录，重装后数据自动恢复
- 如需彻底清除：设置弹窗底部「清除所有本地数据」按钮一键重置，或手动删除 `~/Library/Application Support/com.taskdeck.desktop/` 整个文件夹
...
- **已产出 DMG**（aarch64）：48.7 MB，`npm run build` 一键打包。

...
- **本地 SQLite（WAL）**：数据私有、自用，不上传任何业务数据。数据库文件位于：
  - **安装版**（DMG）：`~/Library/Application Support/com.taskdeck.desktop/taskdeck.db`
  - **开发版**（`npm run dev`）：`app/server/data/taskdeck.db`
  - 卸载或更新 App 不会删除该文件，重装后数据自动恢复。如需彻底清除，手动删除上述路径即可。

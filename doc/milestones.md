...
- **历史版本**：`ChatMsg` 数据模型升级为 `versions[] + activeVersion`（兼容迁移旧 `task`/`tasks`）。重新生成（编辑 / 重试）把上一版任务归档至「已完成任务」，并由二次确认决定是否在对话页保留上一版卡片；保留则 AI 卡下出现「‹ n/m ›」版本切换器回看历次结果。`chatStore` 新增 `editAndRegenerate` / `retry` / `setVersion` / `removeAssistantWithTasks`，复用 `AbortController` 中断与 reconcile 定格。

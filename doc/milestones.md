...
> 参考 ChatGPT 网页交互，给对话卡加悬停操作栏，并支持编辑重生成与历史版本切换。

- **悬停操作栏（图标）**：消息悬停时气泡下方淡入线性图标按钮，替代右键 / 长按菜单。用户气泡：复制 / 编辑 / 删除（仅删本条记录）；AI 卡：重试 / 删除（删整条回复并归档其任务）/ 版本切换。
- **编辑即重新生成**：用户气泡「编辑」→ 气泡原地变可编辑框（白底蓝边、尺寸与原气泡一致，镜像撑宽使宽高随内容扩缩），发送即按新内容重新解析。
- **历史版本**：`ChatMsg` 数据模型升级为 `versions[] + activeVersion`（兼容迁移旧 `task`/`tasks`）。重新生成（编辑 / 重试）把上一版任务归档至「已完成任务」，并由二次确认决定是否在对话页保留上一版卡片；保留则 AI 卡与对应用户气泡下方都出现「‹ n/m ›」版本切换器回看历次结果。`chatStore` 新增 `editAndRegenerate` / `retry` / `setVersion` / `removeAssistantWithTasks`，复用 `AbortController` 中断与 reconcile 定格。
- **「已完成」弹窗底部按钮改永久删除**：「清空所有任务」一键硬删弹窗内全部任务（不可恢复，二次确认写「永久删除」），不再是再归档。
- **已完成态标题删除线**：已完成（绿色 ✓）任务标题加删除线并转灰。
- **文案统一**：归档确认精简为「确定将这条任务归档至『已完成任务』？」；各处归档提示统一为「归档至『已完成任务』」。
- **重构**：移除右键 / 长按 `ctx-*` 菜单及样式（操作栏取代）；对话流双击不选词、选区收束、user-select 白名单保留。

## v1.1.3 — 删除统一为归档 · 已完成回收站 · 输入与选择打磨 ✅ 2026-06-24

> 围绕「删除/归档/恢复」语义统一，并打磨对话输入与文本选择体验。

- **删除语义统一为归档**：四视图 + 对话页任务卡 × 一律软归档（`TaskItem` 二次确认，红色「归档」）；对话卡归档=真归档，全视图同步消失。
- **已完成弹窗合并为回收站**（`CompletedModal` + 新 `ArchivedCard`）：取消已完成/已归档分区，合并一列（已完成在上、已归档在下），每张显示完成状态圆圈 + 恢复（→待办）+ 右上角 × 永久删除（二次确认）。后端新增 `hardDeleteTask` + `DELETE /tasks/:id?hard=1`；`api.purgeTask` / `useArchivedTasks`。
- **二次确认 portal 修层级**：`ConfirmModal` 用 `createPortal` 渲染到 body（`.modal-overlay--confirm` z-index 100），不再被卡片/弹窗遮挡。
- **AI 优先级新增健康底线**（`ai/schema.ts`）：健康任务≥中，当天截止的健康任务≥高。
- **对话气泡操作菜单**：右键/长按弹复制/删除（仅删该条对话记录）。
- **分析可中断**：`AbortController` 串到 `api.createTask`；输入框提示「按 ESC 或点暂停键中断」，发送键转暂停键。
- **输入法选字回车不误发**：`isComposing` 守卫。
- **框选只取文字、不跨卡片**：全局白名单移除 `.bubble` 容器；新增 `lib/confineSelection.ts`（选区跨出起始 `.task-card`/`.bubble` 即收回单元边界）；双击不选词（编辑框除外）。
- **× 位置 / 死代码**：× 固定卡片右上角留 6px；移除失效 `removeChatCard`、`.fab`、`.cal-num*` 等。

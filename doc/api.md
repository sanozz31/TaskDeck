...
| GET | `/tasks/calendar` | 日历范围查（命中 due 或 scheduled） | query: `from,to`（YYYY-MM-DD，必填） | `{tasks: Task[]}` |
...
- **`PATCH /tasks/:id` 白名单**：`title, notes, priority, due_date, due_time, scheduled_date, status, tags`（`tags` 传字符串数组）。其余字段忽略。`status` 改为 `done` 时自动记 `completed_at`，改回其它状态则清空（供 7 天清理）。传 `tags` 时，新出现的标签会并入标签库（`ensureTagDefs`，与 `POST /tasks` 一致）。
